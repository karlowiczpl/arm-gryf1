...
std::map<std::string, std::pair<KomendyAT::enable_fun_ptr, Functions::ErrorStruct>> KomendyAT::goodIdMap ={
    ...
    {"AT+Key" , {Functions::setKey , { 4 , enumFunctions.other , 255}}},
    {"AT+RST" , {Functions::resetGoodId , {0 , enumFunctions.other , 255}}},
    {"AT+SetRGB" , {Functions::rgb , {4 , enumFunctions.other , 255}}},
};
...
void Functions::setOut(STATE_T state) {
  ...
  for(uint8_t i = 0; i < OUTPUTS_COUNT; i++)  eeprom.CommonEeprom.outputsStates[i] = gpio_get_out_level(HardwareInfo.outputs[i]);
...
void Functions::setLed(Functions::STATE_T state) {
  ...
  EepromStruct& eeprom = EepromStruct::getInstance();
  eeprom.CommonEeprom.pwmStates[state.states[1] - 1] = state.states[2];
  eeprom.saveDataToEeprom();
...
void Functions::rgb(Functions::STATE_T state)
{
  pwm_set_gpio_level(HardwareInfo.rgb[state.states[1] - 1][0], state.states[2] * state.states[2]);
  pwm_set_gpio_level(HardwareInfo.rgb[state.states[1] - 1][1], state.states[3]  * state.states[3]);
  pwm_set_gpio_level(HardwareInfo.rgb[state.states[1] - 1][2], state.states[4] * state.states[4]);
  char message[] = "rgb\n";
